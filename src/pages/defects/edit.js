...
export default class Modal extends Component{
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.edit)
      if (props.edit.id !== state.edit.id) {
        return {
          edit: { ...props.edit, pilot: props.edit.pilot?.id, airplane: props.edit.airplane?.id }
        }
      }
    return null
  }
  ...
  save = async e => {
    e.preventDefault()
    const { edit: { id, pilot, airplane, description }} = this.state
    await this.props.save({ id, pilot, description, airplane })
    this.hide()
    this.setState({
      edit:{
        id:"",
              ...
              <div className="modal-body">
                <div className="row">
                  <div className="col-md-6 col-lg-6">
                    <div className="form-group">
                      <select
                        ...
                        required
                        value={this.state.edit.airplane}
                  ...
                  <div className="col-md-12 col-lg-12">
                    <div className="form-group">
                        <textarea class="form-control form-control-alternative" onChange={e => this.setState(Object.assign(this.state.edit, { description: e.target.value}))} rows="3">{this.state.edit.description}</textarea>
                    </div>
                  </div>
                </div>
              </div>
